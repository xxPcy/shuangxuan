...
const buildTeacherStudentMap = (teacher = {}, categories = []) => {
  ...
  categories.forEach((category) => {
    if (!categoryByCode.has(category.code)) categoryByCode.set(category.code, []);
    categoryByCode.get(category.code).push(category);

    const nameKey = String(category.name || '').trim();
    if (!nameKey) return;
    if (!categoryByName.has(nameKey)) categoryByName.set(nameKey, []);
    categoryByName.get(nameKey).push(category);
  });

  const resolveStudentCode = (student = {}) => {
    const raw = [
      student.categoryKey,
      student.specializedCode,
      student.level3_code,
    ...
    const specializedName = String(student.specialized || '').trim();
    if (!specializedName) return '';
    const matched = categoryByName.get(specializedName) || [];
    return matched.length > 0 ? matched[0].code : '';
  };

  const resolveStudentTrack = (student = {}) => {
    const raw = [
      student.track,
  ...
  students.forEach((s) => {
    ...
    if (track) {
      ...
      const candidates = categoryByCode.get(code) || [];
      if (candidates.length === 1) {
        key = candidates[0].key;
      } else {
        const regular = candidates.find((item) => item.track === '全日制');
        key = regular ? regular.key : (candidates[0] ? candidates[0].key : `${code}__全日制`);
      }
    }
